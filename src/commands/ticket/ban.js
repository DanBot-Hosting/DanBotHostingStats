...
module.exports = {
    name: "ban",
    description: "Ban/Unban someone from making a ticket",
    usage: "ban <@user>",
    ...
    run: async (client, message, args) => {
        ...
        if (!userData) {
            ...
            await punishmentsSchema.updateOne({ userId: user.id }, {
                $set: {
                    ticketBanned: !userData.ticketBanned
                }
            });
        }

        message.channel.send(`${user} has been ${userData?.ticketBanned ? "unbanned" : "banned"} from making a ticket.`);
    },
}