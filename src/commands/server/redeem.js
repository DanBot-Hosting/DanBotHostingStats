...
exports.run = async (client, message, args) => {
  let setDonations = async (userid, amount) => {
    await userPrem.set(
      userid + ".used",
      (await userPrem.get(userid + ".used")) || 0
    );
  ...
  if (!args[1]) {
    message.reply(
      "Usage is: `" + Config.DiscordBot.Prefix + "server redeem code`"
    );
    return;
  }
  ...
  let newBal = oldBal + code.balance;
  let now = Date.now();

  message.reply(
    `You have redeemed a code with ${
      code.balance
    } premium server(s), you now have ${Math.floor(
      newBal / Config.PremiumServerPrice
    )}!`
  );
  ...
  client.channels.cache
    ...
    .send(
      `<@${message.author.id}>, Redeemed code: ${args[1]} it held ${
        code.balance
      } premium servers! *This code was redeemed in ${humanizeDuration(
        now - code.createdAt
      )}*`
    );
  ...
  await setDonations(message.author.id, newBal);
...
exports.description =
  "Redeem a premium server code. View commands for more info.";
