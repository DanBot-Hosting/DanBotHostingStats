...
exports.run = async (client, message, args) => {
  return message.channel.send(
    "Server creation is disabled. Do not ping staff. See this announcement: https://discord.com/channels/639477525927690240/898050443446464532/1280611305400569856"
  );
  ...
  const helpEmbed = new Discord.EmbedBuilder()
    ...
    .setFooter({
      text:
        "Example: " +
        Config.DiscordBot.Prefix +
        "server create NodeJS Testing Server",
  ...
  function createServerAndSendResponse(type, message) {
    serverCreateSettings
      ...
      .then((response) => {
        const embed = new Discord.EmbedBuilder().setColor(`Green`).addFields(
          {
            name: "__**Status:**__",
        ...
        if (type === "aio" || type === "java") {
          embed.addFields({
            name: "__**WARNING**__",
            value:
              `Please do not host game servers on java or AIO servers. If you need a Game Server, use a Game Node. Slots are 1$ for` +
              1 / Config.PremiumServerPrice +
              ` servers!`,
          });
        }
