    ...
    ...new Set(
      args
        ...
        .map(
          (id) =>
            id
              .replace(`${Config.Pterodactyl.hosturl}/server/`, "")
              .match(/[0-9a-z]+/i)?.[0]
        )
        .filter(Boolean)
    ),
  ...
  try {
    ...
    Collector.on("collect", async (interaction) => {
      if (interaction.customId === "confirm") {
        await handleServerDeletion(interaction, serversToDelete, message);
        Collector.stop("complete");
      } else if (interaction.customId === "cancel") {
        await interaction.update({
          content: "Server deletion cancelled.",
          components: [],
        });
    ...
    Collector.on("end", (collected, reason) => {
      if (reason === "time") {
        ConfirmMessage.edit({
          content: "Server deletion timed out.",
          components: [],
        });
      }
    });
...
async function handleServerDeletion(interaction, serversToDelete, message) {
  await interaction.update({
    content: `Deleting servers...`,
    components: [],
  });
