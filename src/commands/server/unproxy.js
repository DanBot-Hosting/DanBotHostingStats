...
async function deleteCertificate(Url, Token, CertificateId) {
  return await Axios({
    url: Url + "/api/nginx/certificates/" + CertificateId,
    method: "DELETE",
    followRedirect: true,
    maxRedirects: 5,
    headers: {
...
async function deleteProxy(Url, Token, Domain, ProxiesResponse) {
  return await Axios({
    url:
      ...
      ProxiesResponse.data.find((element) => element.domain_names[0] == Domain)
        .id,
    method: "DELETE",
    followRedirect: true,
    maxRedirects: 5,
...
exports.run = async (client, message, args) => {
  ...
  const domainData = await userDomains.find(
    (x) => x.domain === args[1].toLowerCase()
  );

  if (Proxies.map((x) => x.dbLocation).includes(domainData.location)) {
    //This will contain the proxy information needed to continue.
    const ProxyInformation = Proxies.find(
      (x) => x.dbLocation == domainData.location
    );

    //Generate an new token for the specified location.
    const ProxyToken = await getToken(
      ProxyInformation.url,
      ProxyInformation.email,
      ProxyInformation.pass
    );
    ...
    await Axios({
      ...
      .then(async (ProxiesResponse) => {
        const ProxyFound = ProxiesResponse.data.find(
          (Proxies) => Proxies.domain_names[0] == domainData.domain
        );
        ...
        const CertificateFound = AllCertificates.data.find(
          (element) => element.domain_names[0] == domainData.domain
        );

        const ProxyMessage = await message.channel.send(
          "[PROXY SYSTEM] Attempting to unproxy the domain."
        );

        //If the certificate was found in the certificate list.
        if (CertificateFound == undefined) {
          await ProxyMessage.edit(
        ...
        if (ProxyFound == undefined) {
          ...
          await deleteDomainDB(message.author.id, domainData.domain).then(
            async (Response) => {
              await ProxyMessage.edit(
                ProxyMessage.content +
    ...
    domainData.Location == undefined ||
    domainData.Location == "FR" ||
    domainData.Location == "US"
  ) {
    const ProxyMessage = await message.channel.send(
      "[PROXY SYSTEM] This domain is from an old proxy system. Removing from your database."
    );
