...
exports.description =
  "Delete your panel account and your account data on the bot.";
...
exports.run = async (client, message, args) => {
  ...
  Collector.on("collect", async (Interaction) => {
    if (Interaction.customId === "confirmDelete") {
      await Interaction.update({
        content: "Deleting your panel account...",
        components: [],
      });
      ...
      try {
                ...
                return new Promise((resolve) => {
                  setTimeout(async () => {
                    try {
                      await Axios(
                        deleteServer(
                          `/api/application/servers/${Server.attributes.id}/force`
                        )
                      );
                      resolve();
                    } catch (Error) {
        ...
        await Interaction.followUp({
          content:
            "Your panel account and all associated servers have been successfully deleted.",
          ephemeral: true,
        });
  ...
  Collector.on("end", (collected, reason) => {
    if (reason === "time") {
      MessageReply.edit({
        content: "No response. Account deletion cancelled.",
        components: [],
        embeds: [],
      });
    }
