...
exports.run = async (client, message, args) => {
  ...
  if (userAccount == null) {
    message.reply(
      "You do not have a console account linked with your discord account."
    );
    return;
  }
    ...
    .then((Fetch) => {
      ...
      Axios({
        ...
        .then(async (Response) => {
          ...
          message.author
            .send(
              `New password for DanBot Hosting is below: \`\`\`${data.password}\`\`\``
            )
            .catch((Error) =>
              console.error(
                "[PASSWORD RESET] User does not have direct messages enabled."
              )
            );
          ...
          await sendMail(
            data.email,
            "DanBot Hosting - Password Reset From Discord Bot",
            EmailMessage.html
          ).catch((Error) => {
    ...
    .catch((Error) => {
      if (Error.statusCode == 404) {
        message.channel.send(
          "The account linked to your Discord account does not exist.\n\nUnlink your account using `" +
