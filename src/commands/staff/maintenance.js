...
exports.run = async (client, message, args) => {
  ...
  if (!args[1]) {
    return await message.reply(
      "Please provide a Node to put into maintenance!"
    );
  } else {
    ...
    if (Data == null) {
      return message.reply(
        "Invalid Node provided. Please provide a valid Node DB name."
      );
    } else {
      if (Data.maintenance) {
        ...
        if (!Result)
          return message.reply(
            `Unable to put ${args[1]} out of maintenance mode.`
          );
        ...
        if (!Result)
          return message.reply(
            `Unable to put ${args[1]} into maintenance mode.`
          );
