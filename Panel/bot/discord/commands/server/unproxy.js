...
exports.run = async (client, message, args) => {
    if (!args[1]) {
        ...
        conn.on('ready', function() {
            conn.exec('rm /etc/apache2/sites-enabled/' + args[1] + '.conf && service apache2 restart && echo "complete"', function(err, stream) {
                if (err) throw err;
                stream.on('close', function(code, signal) {
                    ...
                    message.channel.send('FAILED. PLEASE CONTACT HOST ADMIN ASAP')
                });
            });
        })
    }
