...
Router.post(
  "/bot/:ID/stats",
  /* rateLimit(10000, 2) , */ (req, res) => {
    // temp remove if ratelimit
    ...
    if (keys.includes(data.key)) {
      ...
      console.log(
        chalk.magenta("[API] ") + chalk.green(`${data.id} just submitted stats`)
      );
...
Router.get("/astrobot/stats", async (req, res) => {
  axios
    .get("https://astrobot.org/api/stats", {})
  ...
  return rateLimitt({
    ...
    handler: function (req, res) {
      return res.status(429).json({
        ...
        code: 429,
        message:
          "DanBot Hosting Stats API - You are sending too many requests, please slow down",
      });
    },
  });
}
