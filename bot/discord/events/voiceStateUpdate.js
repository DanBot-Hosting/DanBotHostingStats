...
module.exports = async (client, oldV, newV) => {
  ...
  if (oldV.channelID === newV.channelID || enabled.customVoiceChannels == false)
    return;

  if (
    oldV.channelID != null &&
    oldV.channelID != "757660050977456238" &&
    oldV.channel.parentID === "757659750342197289"
  ) {
    if (
      client.pvc.get(oldV.channelID) != null &&
      client.pvc.get(oldV.channelID).owner == oldV.member.id
    ) {
      oldV.channel.delete();
