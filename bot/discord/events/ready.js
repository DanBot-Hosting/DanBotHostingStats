...
module.exports = async (client) => {
    ...
    guild.members.cache
      .filter((member) =>
        [
          ...
          "[",
          "]",
        ].some((r) => member.displayName.startsWith(r))
      )
      .forEach((x) => {
  ...
  setInterval(async () => {
    ...
    client.channels.cache.get("815242427978743848").edit({
      name: `Total Members: ${
        client.guilds.cache.get("639477525927690240").memberCount
      }`,
      reason: "TMembers count update",
    });

    let guild4 = await client.guilds.cache.get("639477525927690240");
    const ticketcount = guild4.channels.cache.filter((x) =>
      x.name.endsWith("-ticket")
    ).size;
    ...
    client.channels.cache.get("758746579636191382").edit({
      name: `Boosts: ${
        client.guilds.cache.get("639477525927690240").premiumSubscriptionCount
      }`,
  ...
  setInterval(() => {
    axios({
      url:
        config.DanPterodactyl.hosturl +
        "/api/client/servers/019b6467/resources",
      method: "GET",
      followRedirect: true,
      maxRedirects: 5,
      headers: {
