let parse = () => {
  ...
  for (let channel of channels) {
    ...
    messages.forEach((message) => {
      for (let [reaction, role] of Object.entries(
        config.DiscordBot.reactionRoles[channel][message]
      )) {
        toReturn.push({
          message: message,
          reaction: reaction,
...
module.exports = async (client, r, member) => {
  ...
  let found = reactionRole.find(
    (x) => x.message == r.message.id && x.reaction == emoji
  );
  ...
  if (member.roles.cache.get(found.role) != null) {
    ...
    let memberRoles = member.roles.cache
      .map((x) => x.id)
      .concat(reactionRole.map((x) => x.role));
