  ...
  if (enabled.Welcome === true) {
    ...
    if (Date.now() - member.user.createdAt < 863136000) {
      await member.user.send(
        `Sorry! We only allow accounts over the age of 10days to join. \nYour account was created ${humanizeDuration(
      ...
      welcomeChannel.send(
        member.user.tag +
          ` has been auto-kicked as account is under 10days old.\nThat account was created ${humanizeDuration(
            Date.now() - member.user.createdAt,
            { round: true }
          )}, ago`
      );
    ...
    if (member.user.bot) {
      ...
      if (!bot) {
        welcomeChannel.send(
          "Bot: <@" + member + ">, tried to join but is not using our API."
        );
        member.kick();
      } else {
    ...
    if (userData.get(member.id) == null) {
      welcomeChannel.send(
        "Welcome <@" +
          member +
          "> to DanBot Hosting. To get started please read <#738527470164377630>"
      );
    } else {
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .addField(
        "Members Status",
        member.user.presence !== null && member.user.presence.status !== null
    ...
    let inviteChannel = client.channels.cache.get(
      config.DiscordBot.inviterewmsg
    );

    if (
      Object.keys(config.DiscordBot.invites).includes(invite.uses.toString())
    ) {
      await inviter.removeRoles(Object.values(config.DiscordBot.invites));
      inviter
        .addRole(config.DiscordBot.invites[invite.uses.toString()])
        .then(() => {
          inviteChannel.send(
            `<@${inviter.id}> just hit ${invite.uses} invites! Here's a role for you :)`
          );
        });
    }
  }
