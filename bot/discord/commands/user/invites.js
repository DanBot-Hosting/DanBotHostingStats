  ...
  message.guild
    ...
    .then((invites) => {
      const userInvites = invites
        .array()
        .filter((o) => o.inviter.id === targetUser.id);
      ...
      if (isAnotherUserLookup)
        message.channel.send(
          `User \`_${targetUser.username}_\` has invited ${userInviteCount} users`
        );
      else
        message.reply(
          `You have invited ${userInviteCount} users to this server. `
        );
    })
    .catch(console.error);
};
