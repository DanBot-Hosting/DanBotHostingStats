exports.run = async (client, message, args) => {
    if (message.guild.channels.cache.find(channel => channel.name === `${message.author.username.toString().toLowerCase() + message.author.discriminator}-ticket`)) return message.channel.send('You already have a ticket open!')
    const server = message.guild

    let channel = await server.channels.create(message.author.tag + "-Ticket", "text", [{
        type: 'role',
        id: message.guild.id,
        deny: 0x400
    },
