      ...
      if (!channel) {
        ...
        let giveawayMessage = await message.channel.send("", {
          embed: new Discord.MessageEmbed()
            .setTitle("GIVEAWAY! 🎉")
            .setDescription(
              `Giveaway event started by <@${message.author.id}>. React to this message with ${reaction} to get a chance to win **${prize}**.`
            )
            .setColor("#0000ff")
            .setFooter(
              `Event stops in ${time}. You will get your reward after the event has concluded.`
            ),
        });
        ...
        giveaway = bot.setTimeout(async () => {
          let giveawayMessage = await message.channel.messages.fetch(
            giveawayMessageID
          );

          let winners = [];
          if (giveawayMessage.reactions.get(reaction)) {
            winners = giveawayMessage.reactions
              .get(reaction)
            ...
            winner
              .send("", {
                embed: new Discord.MessageEmbed()
                  ...
                  .setDescription(
                    `You won the giveaway in **${message.guild.name}** Server! And you've been awarded with **${prize}**!`
                  )
                  .setColor("#0000ff"),
              })
              .catch(() => {});
            ...
            giveawayMessage
              .edit("", {
                embed: new Discord.MessageEmbed()
                  .setTitle("Giveaway Event Ended")
                  .setDescription(
                    `Unfortunately, no one participated and apparently there\'s no winner. 😕`
                  )
                  .setColor("#ff0000"),
              })
              .catch((e) => {
          ...
          message.channel
            .send("", {
              embed: new Discord.MessageEmbed()
                .setTitle("Giveaway Event Ended")
                .setDescription(
                  `The giveaway event was abruptly ended by ${message.author.tag}. Sorry, no giveaways this time!`
                )
                .setColor("#ff0000"),
            })
            .catch((e) => {
