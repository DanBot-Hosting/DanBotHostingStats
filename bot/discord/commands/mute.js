...
exports.run = async (client, message, args) => {
  ...
  if (
    ...
    return message.reply(
      "sorry, but it looks like you're too much of a boomer to run this command."
    );
  if (!message.guild.me.hasPermission("MANAGE_ROLES"))
    return message.reply(
      "Sorry, i dont have the perms to do this cmd i need MANAGE_ROLES. :x:"
    );
  ...
  let target = message.guild.members.cache.get(
    args[0].match(/[0-9]{18}/).length == 0
      ? args[0]
      : args[0].match(/[0-9]{18}/)[0]
  );
