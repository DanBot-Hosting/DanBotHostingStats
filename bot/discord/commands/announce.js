...
const parse = (string, options) => {
  ...
  flags.forEach((x) => {
    ...
    if (!flagsToUse.includes(args[0].toLowerCase())) {
      ...
      toReturn[toReturnKeys[toReturnKeys.length - 1]] =
        toReturn[toReturnKeys[toReturnKeys.length - 1]] + " -" + args.join(" ");
      return;
    }

    let flag = args.shift().toLowerCase();

    if (args.length == 0) if (flag) args = true;
...
exports.run = async (client, message, args) => {
  if (!message.member.roles.cache.find((r) => r.id === "639489438036000769"))
    return;
