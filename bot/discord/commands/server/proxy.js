...
exports.run = async (client, message, args) => {
  const embed = new Discord.MessageEmbed().setTitle(
    "__**How to link a domain to a website/server**__ \nCommand format: " +
    ...
    if (!args[2]) {
      ...
      const linkalready = userData
        ...
        .filter(
          (users) =>
            users.data.domains &&
            users.data.domains.filter((x) => x.domain === args[1]).length != 0
        );
      if (linkalready[0]) {
        ...
        axios({
          ...
          if (use.relationships) {
            ...
            k.forEach((key) => {
              if (use.relationships[key].data != null)
                use.extras[key] = use.relationships[key].data.map(
                  (a) => a.attributes
                );
          ...
          if (
            use.extras.servers == null ||
            use.extras.servers.find((x) => x.identifier === args[2]) == null
          ) {
              ...
              .then((sentmsg) => {
                ...
                conn.on("ready", function () {
                  conn.exec(
                    "certbot certonly -d " +
                      args[1] +
                      " --non-interactive --webroot --webroot-path /var/www/html --agree-tos -m proxy@danbot.host",
                    function (err, stream) {
                      if (err) throw err;
                      stream
                          ...
                          if (data.includes("Congratulations!")) {
                            ...
                            conn.exec(
                              `echo "<VirtualHost *:80>
                                             ...
                                             </VirtualHost>" > /etc/apache2/sites-enabled/${args[1]}.conf && sleep 1 && echo "complete`,
                              function (err, stream) {
                                if (err) throw err;
                                stream
                                  .on("close", function (code, signal) {})
                                  .on("data", function (data) {
                                    ...
                                    setTimeout(() => {
                                      conn.exec(
                                        'service apache2 restart && echo "complete',
                                        function (err, stream) {
                                          if (err) throw err;
                                          stream
                                            .on(
                            ...
                            data.includes("Certificate not yet due for renewal")
                          ) {
                            sentmsg.edit(
                              "Please give me a few seconds! \nProcess: SSL Complete. \nNext: Write proxy file. **Sometimes this gets stuck, If it takes more than 10seconds run the command again**"
                            );
                            ...
                            sentmsg.edit(
                              "ERROR, SSL failed to connect. Is your domain pointing to the correct ip address? \nReverse Proxy ip is: `" +
                                config.SSH.Host +
                                "`"
                            );
                          }
