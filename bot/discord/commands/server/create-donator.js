  ...
  axios({
    ...
    .then(() => {
      const serverName =
        ...
        "change me! (Settings -> SERVER NAME)";
      ...
      let allowed =
        message.member.roles.cache.get("710208090741539006") != null
          ? Math.floor(user.donated / config.node7.price) +
            (boosted != null ? Math.floor(boosted * 2.5) : 2)
          : Math.floor(user.donated / config.node7.price);
      ...
      if (client.cooldown[message.author.id].pCreate > Date.now()) {
        message.reply(
          `You're currently on cooldown, please wait ${humanizeDuration(
            client.cooldown[message.author.id].pCreate - Date.now(),
            { round: true }
          )}`
        );
        return;
      ...
      message.channel.send(
        new Discord.MessageEmbed()
          ...
          .setFooter(
            "Example: " +
              config.DiscordBot.Prefix +
              "server create-donator NodeJS Testing Server"
          )
      );
    });
};
